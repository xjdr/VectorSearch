...
#include "kdt_parameter_definition_list.h"
...
#pragma omp parallel for
    for (int i = 0; i < GetNumSamples(); i++) {
      Utils::Normalize(m_pSamples[i], GetFeatureDim(), base);
    }
  }
  ...
  int begin, end;
  {
    ...
    if (DistCalcMethod::Cosine == m_iDistCalcMethod) {
      ...
      for (int i = begin; i < end; i++) {
        Utils::Normalize((T*)m_pSamples[i], GetFeatureDim(), base);
      }
    }
  }
...
#define DefineKDTParameter(VarName, VarType, DefaultValue, RepresentStr)  \
  else if (vsearch::Helper::StrUtils::StrEqualIgnoreCase(p_param,           \
                                                       RepresentStr)) {   \
    fprintf(stderr, "Setting %s with value %s\n", RepresentStr, p_value); \
    VarType tmp;                                                          \
    if (vsearch::Helper::Convert::ConvertStringTo<VarType>(p_value, tmp)) { \
      VarName = tmp;                                                      \
    }                                                                     \
  }
...
#define DefineKDTParameter(VarName, VarType, DefaultValue, RepresentStr) \
                                                       ...
                                                       RepresentStr)) {  \
    return vsearch::Helper::Convert::ConvertToString(VarName);             \
  }
...
#define DefineVectorValueType(Name, Type) \
  template class vsearch::KDT::Index<Type>;
